...
bscols(
  ...
  list(
    ...
    filter_select("position", "Position", underdog_shared, ~Position),
    #filter_slider("exposure", "Exposure", underdog_shared  , ~Exposure,  width = "100%"),
    #filter_slider("weighted ADP", "Weighted_ADP", underdog_shared, ~Weighted_ADP, width = "100%"),
    filter_select("team", "Team", underdog_shared, ~Team)
  ),
...
ggplotly(underdog_shared %>%  ggplot(aes(x = Weighted_ADP, y = Exposure  ,color = Position , label = Player)) +
     geom_point(aes(text = str_glue("Player: {Player}")   ) , size = 0.8)+ 
  theme_tq() + scale_color_tq() +
      geom_text(check_overlap = TRUE , size = 2 , nudge_y = 1) + theme(legend.position="none")  ) 
