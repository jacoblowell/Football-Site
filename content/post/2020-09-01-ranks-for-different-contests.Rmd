...
Underdog_Ownership_Static  <- Underdog_Ownership %>%  group_by(Team, Position, Player)  %>%  
  ...
  ungroup()  %>%  select(  - Earliest , - Latest , -ADP ) 



Underdog_Ownership_Static %>%  relocate(Range, .after = last_col()) %>%  select(-Times_Drafted) %>%   reactable( columns = list(
...
Underdog_Ownership_Static %>% filter(Position == "QB")  %>%  relocate(Range, .after = last_col()) %>%  select(-Times_Drafted , -Position , -With_QB_Rate) %>%   reactable( columns = list(
 Team = colDef(minWidth = 50)  ,
Exposure = colDef(minWidth = 75 , format = colFormat(
  prefix = "$",
  separators = TRUE,
